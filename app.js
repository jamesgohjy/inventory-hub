// AV Inventory Hub V6.88 — Line-item completeness and evidence reconfirmation
...
const V682_DEPLOYMENT_BUILD='6.88.0';
function v682MarkDeployment(){
  try{
    window.__AV_INVENTORY_VERSION__='6.88';
    ...
    if(cv)cv.textContent='v6.88';
    if(av)av.textContent='Version 6.87';
    document.documentElement.dataset.avInventoryVersion='6.88';
...
function v682AttachNormalization(parsed={},sourceText=''){
  const normalized=v682NormalizeInvoice(parsed,sourceText);return{...parsed,normalizedInvoice:normalized,normalizedMarkdown:v682InvoiceMarkdown(normalized),parseEvidence:{...(parsed.parseEvidence||{}),normalization_schema:'1.0',normalization_parser:'6.88'}};
}
...
async function launch(){
  try{
    ...
    src=replaceOnce(src,"// AV Inventory Hub V6.55 — evidence-only inventory parsing and verified invoice dates","// AV Inventory Hub V6.88 — Line-item completeness and evidence reconfirmation",'version header');
...
function v667EnsurePatchNotesUi(){
  try{
    ...
    if(cv)cv.textContent='v6.88';if(av)av.textContent='Version 6.87';
