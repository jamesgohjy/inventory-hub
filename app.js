// AV Inventory Hub V6.78 — no-AUTO SKU enforcement and self-rectifying invoice validation
...
function v661FinalizeParsedInvoice(parsed={},raw=''){
  ...
  candidates.sort((a,b)=>b.score-a.score);const extracted=candidates[0]?.items||[],withSerials=v677ValidateInvoiceLines(attachSerialBlocks(extracted,evidence),evidence),inventory=sanitizeParsedInventoryItems(inventoryOnlyItems(withSerials),evidence);
...
async function launch(){
  try{
    ...
    src=replaceOnce(src,"// AV Inventory Hub V6.55 — evidence-only inventory parsing and verified invoice dates","// AV Inventory Hub V6.78 — no-AUTO SKU enforcement and self-rectifying invoice validation",'version header');
    src=replaceOnce(src,"const APP_VERSION='6.55';","const APP_VERSION='6.78';",'APP_VERSION');
...
`,'date-company source filename');
    ...
    console.error('AV Inventory Hub V6.78 startup error:',err);
    ...
    box.innerHTML='<b>AV Inventory Hub V6.78 could not start.</b><br>The verified V6.55 base was left untouched in Git history.<br><br><code>'+String(err.message||err).replace(/[&<>]/g,s=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[s]))+'</code>';
...
function v667EnsurePatchNotesUi(){
  try{
    window.__AV_INVENTORY_VERSION__='6.78';
    ...
    if(cv)cv.textContent='v6.78';if(av)av.textContent='Version 6.78';
    if(notes&&!notes.children.length)notes.innerHTML=V667_RELEASE_NOTES.map(x=>'<li>'+x.replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]))+'</li>').join('');
  }catch(e){console.warn('V6.78 patch-notes fallback skipped',e);}
}
