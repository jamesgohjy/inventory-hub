...
async function launch(){
  ...
  const ext=(sourceName.match(/\.(pdf|png|jpe?g|webp|docx)$/i)||[])[1]||'pdf';
  return [date,supplier,invoice].filter(Boolean).join('_')+'.'+ext.replace('jpeg','jpg');
}
...
`,'multi-format source filename preservation');
