// AV Inventory Hub v7.03.3.14r — repository-static runtime bootstrap
(function(){
  'use strict';
  if(window.__AV_V703314R_BOOTSTRAP_STARTED__)return;
  window.__AV_V703314R_BOOTSTRAP_STARTED__=true;
  const VERSION='7.03.3.14r',ASSET_REV='v703314r-static-baseline-20260924-1';
  ...
  async function launch(){
    try{
      ...
      await loadScript('runtime-v7.03.3.14r.js?v='+key);
      if(!window.__AV_STATIC_RUNTIME_READY__?.then)throw new Error('Static runtime readiness promise was not created.');
      await window.__AV_STATIC_RUNTIME_READY__;if(window.__AV_STATIC_RUNTIME_LOADED__!==VERSION)throw new Error('Static runtime did not initialise as '+VERSION+'.');
      v7032.installParserPatch();v7033.installParserPatch();v7033.installUiVersionSync();
      window.__AV_INVENTORY_VERSION__=VERSION;window.__AV_INVENTORY_BUILD__=VERSION;window.__AV_INVENTORY_BASELINE__='repository-local d452 baseline + static 14r derived runtime';
      console.info('AV Inventory Hub '+VERSION+' loaded from repository-local runtime.',Object.fromEntries(gates));
    }catch(err){
