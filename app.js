// AV Inventory Hub V6.84 — Multi-candidate line-item reconciliation
...
const V682_DEPLOYMENT_BUILD='6.84.0';
function v682MarkDeployment(){
  try{
    window.__AV_INVENTORY_VERSION__='6.84';
    ...
    if(cv)cv.textContent='v6.84';
    if(av)av.textContent='Version 6.84';
    document.documentElement.dataset.avInventoryVersion='6.84';
...
async function launch(){
  try{
    ...
    src=replaceOnce(src,"// AV Inventory Hub V6.55 — evidence-only inventory parsing and verified invoice dates","// AV Inventory Hub V6.84 — Multi-candidate line-item reconciliation",'version header');
    src=replaceOnce(src,"const APP_VERSION='6.55';","const APP_VERSION='6.84';",'APP_VERSION');
...
`,'date-company source filename');
    ...
    console.error('AV Inventory Hub V6.84 startup error:',err);
    ...
    box.innerHTML='<b>AV Inventory Hub V6.84 could not start.</b><br>The verified V6.55 base was left untouched in Git history.<br><br><code>'+String(err.message||err).replace(/[&<>]/g,s=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[s]))+'</code>';
...
function v667EnsurePatchNotesUi(){
  try{
    ...
    if(cv)cv.textContent='v6.84';if(av)av.textContent='Version 6.84';
