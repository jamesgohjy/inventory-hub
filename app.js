// AV Inventory Hub v7.03.3.1 — freeze hotfix + cumulative patch loader
...
(function(){
  ...
  async function launch(){
    try{
      ...
      src+=`\n
;try{globalThis.V7032Patch?.installParserPatch();globalThis.V7033Patch?.installParserPatch();globalThis.V7033Patch?.installUiVersionSync();}catch(e){console.warn('v7.03.3.1 post-launch patch install skipped',e);}

function v7033PrepareImportLines(lines=[]){
  try{return globalThis.V7033Patch?.prepareLinesForInventory(lines,state?.data?.items||[])||lines;}catch(e){console.warn('v7.03.3.1 inventory match preparation skipped',e);return lines;}
}
(function v7033WrapImports(){
  ...
  }catch(e){console.warn('v7.03.3.1 import matching wrapper skipped',e);}
})();
...
async function v7033AutoWebVerify(){
  ...
  }catch(e){console.warn('v7.03.3.1 Level 2B web verification skipped',e);}
}
...
\n`;
      ...
      console.error('AV Inventory Hub v7.03.3.1 startup error:',err);
      ...
      const msg=String(err?.message||err).replace(/[&<>]/g,s=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[s]));box.innerHTML='<b>AV Inventory Hub v7.03.3.1 could not start.</b><br>No database changes were made by this loader.<br><br><code>'+msg+'</code>';document.body.appendChild(box);
    }
  }
  launch();
})();
