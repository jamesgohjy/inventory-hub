// AV Inventory Hub V6.83 — Admin import diagnostics + AV Media evidence reconciliation
...
function v682MarkDeployment(){
  try{
    ...
    if(cv)cv.textContent='v6.83';
    if(av)av.textContent='Version 6.83';
...
function v682AttachNormalization(parsed={},sourceText=''){
  const normalized=v682NormalizeInvoice(parsed,sourceText);return{...parsed,normalizedInvoice:normalized,normalizedMarkdown:v682InvoiceMarkdown(normalized),parseEvidence:{...(parsed.parseEvidence||{}),normalization_schema:'1.0',normalization_parser:'6.83'}};
}
...
async function launch(){
  try{
    ...
    src=replaceOnce(src,"// AV Inventory Hub V6.55 — evidence-only inventory parsing and verified invoice dates","// AV Inventory Hub V6.83 — Admin import diagnostics + AV Media evidence reconciliation",'version header');
    src=replaceOnce(src,"const APP_VERSION='6.55';","const APP_VERSION='6.83';",'APP_VERSION');
...
`,'date-company source filename');
    ...
    console.error('AV Inventory Hub V6.82 startup error:',err);
    ...
    box.innerHTML='<b>AV Inventory Hub V6.83 could not start.</b><br>The verified V6.55 base was left untouched in Git history.<br><br><code>'+String(err.message||err).replace(/[&<>]/g,s=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[s]))+'</code>';
...
function v667EnsurePatchNotesUi(){
  try{
    ...
    if(cv)cv.textContent='v6.83';if(av)av.textContent='Version 6.83';
