...
function v703RenderVerificationNotice(){
  ...
  const v7=state.parsed?.v7||state.parsed?.parseEvidence?.v7||{};
  const comp=v7.completenessValidation||state.parsed?.parseEvidence?.completeness||{};
  const rows=v7.verification?.humanReviewRows||[];
  const needs=!!(v7.humanReviewRequired||comp.recheckRequired||rows.length);
  if(!needs){box.classList.add('hidden');box.textContent='';return;}
  const reasons=[];
  if(comp.recheckRequired)reasons.push(`Line-item completeness: expected ${comp.expectedEquipmentCount??'?'}, final ${comp.finalEquipmentCount??'?'}.`);
  for(const r of rows.slice(0,4))reasons.push(r.reason||'Independent verification could not confirm a row.');
  box.classList.remove('hidden');
  box.innerHTML='<b>Level 3 — Human review required</b><br>Level 1 and Level 2 could not fully agree. Compare the Parsed Fields with the PDF and correct any uncertain values before saving.'+(reasons.length?'<br><small>'+reasons.map(x=>'• '+String(x).replace(/[&<>]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[c]))).join('<br>')+'</small>':'');
}
