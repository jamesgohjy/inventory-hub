// AV Inventory Hub v7.03.3 — cumulative patch loader
...
(function(){
  ...
  async function launch(){
    try{
      ...
      src+=`\n
;try{globalThis.V7032Patch?.installParserPatch();globalThis.V7033Patch?.installParserPatch();globalThis.V7033Patch?.installUiVersionSync();}catch(e){console.warn('v7.03.3 post-launch patch install skipped',e);}

function v7033PrepareImportLines(lines=[]){
  try{return globalThis.V7033Patch?.prepareLinesForInventory(lines,state?.data?.items||[])||lines;}catch(e){console.warn('v7.03.3 inventory match preparation skipped',e);return lines;}
}
(function v7033WrapImports(){
  ...
  }catch(e){console.warn('v7.03.3 import matching wrapper skipped',e);}
})();
...
async function v7033AutoWebVerify(){
  try{
    ...
    const result=await globalThis.V7032WebVerify.verifyParsed(state.parsed,raw,{timeoutMs:7000});state.parsed=result.parsed;
  ...
  }catch(e){console.warn('v7.03.3 Level 2B web verification skipped',e);}
}
...
async function v7033ConsolidateExistingSafeDuplicates(){
  ...
  try{const {data,error}=await state.db.sb.rpc('consolidate_inventory_duplicates_v7033');if(error){if(/function|schema cache|not found|PGRST202|42883/i.test(String(error.message||'')))return;throw error;}const merged=Number(data?.merged_items||0);if(merged>0){state.data=await state.db.load();renderAll();toast('Consolidated '+merged+' verified duplicate inventory record'+(merged===1?'':'s')+'.');}}catch(e){console.warn('v7.03.3 duplicate consolidation skipped',e);}
}
try{const originalReload=reload;reload=async function(){const x=await originalReload();setTimeout(v7033ConsolidateExistingSafeDuplicates,50);return x;};}catch(e){console.warn('v7.03.3 reload consolidation hook skipped',e);}
setTimeout(v7033ConsolidateExistingSafeDuplicates,1800);
\n`;
      ...
      console.info('AV Inventory Hub v'+VERSION+' loaded cumulatively from live v7.03.2 logic.');
    }catch(err){
      console.error('AV Inventory Hub v7.03.3 startup error:',err);
      ...
      const msg=String(err?.message||err).replace(/[&<>]/g,s=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[s]));box.innerHTML='<b>AV Inventory Hub v7.03.3 could not start.</b><br>No database changes were made by this loader.<br><br><code>'+msg+'</code>';document.body.appendChild(box);
    }
  }
  launch();
})();
