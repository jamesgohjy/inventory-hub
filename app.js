// AV Inventory Hub v7.03.3.3 — freeze hotfix + cumulative patch loader
...
(function(){
  ...
  async function launch(){
    try{
      ...
      src+=`\n
;try{globalThis.V7032Patch?.installParserPatch();globalThis.V7033Patch?.installParserPatch();globalThis.V7033Patch?.installUiVersionSync();}catch(e){console.warn('v7.03.3.3 post-launch patch install skipped',e);}

function v7033PrepareImportLines(lines=[]){
  try{return globalThis.V7033Patch?.prepareLinesForInventory(lines,state?.data?.items||[])||lines;}catch(e){console.warn('v7.03.3.3 inventory match preparation skipped',e);return lines;}
}
(function v7033WrapImports(){
  ...
  }catch(e){console.warn('v7.03.3.3 import matching wrapper skipped',e);}
})();
...
async function v7033AutoWebVerify(){
  ...
  }catch(e){console.warn('v7.03.3.3 Level 2B web verification skipped',e);}
}
...
// v7.03.3.3 Documents metadata editor. Runs inside the baseline module so it can safely
...
function v70333OpenDocumentEdit(id){if(!requireEdit())return;const x=state.data.documents.find(d=>String(d.id)===String(id));if(!x)return;v70333EnsureEditDialog();document.getElementById('v70333DocId').value=x.id;document.getElementById('v70333Supplier').value=x.supplier_name||'';document.getElementById('v70333Invoice').value=x.invoice_number||'';document.getElementById('v70333Date').value=String(x.invoice_date||'').slice(0,10);document.getElementById('v70333DocEditDialog').showModal();}
...
\n`;
      ...
      console.error('AV Inventory Hub v7.03.3.3 startup error:',err);
      ...
      const msg=String(err?.message||err).replace(/[&<>]/g,s=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[s]));box.innerHTML='<b>AV Inventory Hub v7.03.3.3 could not start.</b><br>No database changes were made by this loader.<br><br><code>'+msg+'</code>';document.body.appendChild(box);
    }
  }
  launch();
})();
