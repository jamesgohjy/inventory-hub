// AV Inventory Hub V6.89 — Parser reliability and regression protection
...
const V682_DEPLOYMENT_BUILD='6.89.0';
function v682MarkDeployment(){
  try{
    window.__AV_INVENTORY_VERSION__='6.89';
    ...
    if(cv)cv.textContent='v6.89';
    if(av)av.textContent='Version 6.89';
    document.documentElement.dataset.avInventoryVersion='6.89';
...
function v674IsExcludedInventoryAccessoryLine(x={}){
  ...
  // V6.89: microphone stands are tracked inventory. This specific exception must run before generic stand exclusions.
...
function v689SerialIntegrityGate(items=[],sourceText=''){
  const out=(items||[]).map(x=>({...x})),seen=new Map();
...
function v682AttachNormalization(parsed={},sourceText=''){
  const normalized=v682NormalizeInvoice(parsed,sourceText);return{...parsed,normalizedInvoice:normalized,normalizedMarkdown:v682InvoiceMarkdown(normalized),parseEvidence:{...(parsed.parseEvidence||{}),normalization_schema:'1.0',normalization_parser:'6.89'}};
}
...
async function launch(){
  try{
    ...
    src=replaceOnce(src,"// AV Inventory Hub V6.55 — evidence-only inventory parsing and verified invoice dates","// AV Inventory Hub V6.89 — Parser reliability and regression protection",'version header');
    src=replaceOnce(src,"const APP_VERSION='6.55';","const APP_VERSION='6.89';",'APP_VERSION');
...
function v667EnsurePatchNotesUi(){
  try{
    ...
    if(cv)cv.textContent='v6.89';if(av)av.textContent='Version 6.89';
