// Inventory Hub direct application runtime — v7.03.3.14t
...
function v661FinalizeParsedInvoice(parsed={},raw=''){
  ...
  const sourceBoost=o=>/^header-aligned:/i.test(o)?980:/stacked-priced:recovery-block/i.test(o)?760:/stacked-priced:recovery-auto/i.test(o)?720:/stacked-priced:recovery-column/i.test(o)?680:/stacked-priced:recovery-sparse/i.test(o)?620:/strict-layout:recovery-column/i.test(o)?700:/strict-layout:recovery-auto/i.test(o)?600:/strict-layout:recovery-block/i.test(o)?550:/strict-layout:recovery-sparse/i.test(o)?450:/strict-layout:server/i.test(o)?350:0;
  ...
  const strictCandidates=candidates.filter(c=>(String(c.origin||'').startsWith('header-aligned:')||String(c.origin||'').startsWith('strict-layout:'))&&c.items?.length&&c.verifiedRows>0&&c.badRows===0);
...
(function v703314qInstallMoneyAndGroupHardening(){
  ...
  const api=globalThis.V7033Patch,diagnostics=window.__AV_14Q_DIAGNOSTICS__={version:'7.03.3.14t',installed:false,stage:'boot',error:null};
  try{
  ...
  }catch(err){diagnostics.stage='failed';diagnostics.error=String(err?.message||err);console.warn('V7.03.3.14t money/group hardening failed non-fatally.',err);}
})();
...
try{installParseAuditWrappers();configureInvoiceFileInputs();}catch(e){console.warn('V6.69 optional audit/file-input setup skipped',e);}
  ...
  window.__AV_DIRECT_RUNTIME_LOADED__='7.03.3.14t';
  return true;
})().catch(err=>{window.__AV_DIRECT_RUNTIME_ERROR__=String(err?.message||err);console.error('AV Inventory Hub v7.03.3.14t direct runtime failed',err);throw err;});
