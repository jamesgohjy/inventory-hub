...
(function(global){
  ...
  function renderGroupedCompanyCards({host,groups,head,escapeHtml}={}){
    ...
    host.innerHTML='<div class="v669-doc-groups">'+[...groups.entries()].sort((a,b)=>a[0].localeCompare(b[0],undefined,{sensitivity:'base'})).map(([company,list])=>`<section class="v669-doc-group inventory-company-group ui-group"><button type="button" class="v669-doc-group-toggle ui-group__toggle" aria-expanded="false"><strong>${escapeHtml(company)}</strong><span>${list.length} item${list.length===1?'':'s'} ▸</span></button><div class="v669-doc-group-body ui-group__body hidden"><table>${head}<tbody>${list.map(x=>x.html).join('')}</tbody></table></div></section>`).join('')+'</div>';
