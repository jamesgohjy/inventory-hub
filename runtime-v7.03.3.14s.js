// Inventory Hub direct application runtime — v7.03.3.14s
...
(function v703314qInstallMoneyAndGroupHardening(){
  ...
  const api=globalThis.V7033Patch,diagnostics=window.__AV_14Q_DIAGNOSTICS__={version:'7.03.3.14s',installed:false,stage:'boot',error:null};
  try{
  ...
  }catch(err){diagnostics.stage='failed';diagnostics.error=String(err?.message||err);console.warn('V7.03.3.14s money/group hardening failed non-fatally.',err);}
})();
...
try{installParseAuditWrappers();configureInvoiceFileInputs();}catch(e){console.warn('V6.69 optional audit/file-input setup skipped',e);}
  ...
  window.__AV_DIRECT_RUNTIME_LOADED__='7.03.3.14s';
  return true;
})().catch(err=>{window.__AV_DIRECT_RUNTIME_ERROR__=String(err?.message||err);console.error('AV Inventory Hub v7.03.3.14s direct runtime failed',err);throw err;});
