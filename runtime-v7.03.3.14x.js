...
(function v703314oInstallParserIntelligence(){
  ...
  try{
    ...
    async function clearCorrections14x(){if(!isAdmin14o())return;if(!confirm('Clear all Correction Memory entries? This cannot be undone. Supplier Layout Profiles and parser quality events will not be deleted.'))return;if(CFG.mode==='supabase'&&state?.db?.sb)await rpc14o('clear_parser_correction_memory_v703314x',{});else{const d=localData();d.corrections=[];saveLocal(d);}await refreshIntelligence14o();toast('Correction Memory cleared.');}
