...
$('parsedItems').addEventListener('input',e=>{
  ...
  const q=Number(qty.value),p=Number(price.value);if(!(q>0)||!Number.isFinite(p)||p<0)return;
  amount.value=(Math.round(q*p*100)/100).toFixed(2);amount.dataset.autoCalculated='true';
},true);
