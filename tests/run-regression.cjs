...
assert(parserV2Verification.includes("if(exact||n>70)return 'confirmed'")&&parserV2Verification.includes("if(n<60)return 'rejected'")&&!parserV2Verification.slice(verifyOnePos,parserV2Verification.indexOf('async function mapLimit',verifyOnePos)).includes("if(inv.status==='rejected')return"),'Inventory similarity may classify a local match but must not reject equipment before mandatory web verification');
...
assert(index.includes('app.js?v=7.03.3.14y-r20'),'Index app.js cache-bust revision missing');
